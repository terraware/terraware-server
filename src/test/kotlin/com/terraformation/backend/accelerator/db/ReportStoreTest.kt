...
class ReportStoreTest : DatabaseTest(), RunsAsDatabaseUser {
  ...
  inner class UnpublishedProperties {
    ...
    fun `returns empty list when report has never been published`() {
      val model = store.fetchOne(reportId)
      assertEquals(
          emptyList<PublishedReportComparedProps>(),
          model.unpublishedProperties,
          "unpublishedProperties should be empty",
      )
    }
    ...
    fun `returns empty list when report matches published qualitative properties`() {
      ...
      assertEquals(
          emptyList<PublishedReportComparedProps>(),
          store.fetchOne(reportId).unpublishedProperties,
    ...
    fun `identifies all changed properties`() {
      ...
      assertEquals(
          ...
          store.fetchOne(reportId, includeIndicators = true).unpublishedProperties.toSet(),
    ...
    fun `does not identify commonIndicators as changed for non-publishable indicators`() {
      ...
      val model = store.fetchOne(reportId, includeIndicators = true)
      assertEquals(
          emptyList<PublishedReportComparedProps>(),
          model.unpublishedProperties,
    ...
    fun `returns correct unpublishedProperties per report when fetching multiple reports`() {
      ...
      val reports =
          store.fetch(projectId = projectId, includeFuture = true, includeIndicators = true)
