...
class DefaultVoterStoreTest : DatabaseTest(), RunsAsUser {
  ...
  inner class Insert {
    ...
    @Test
    fun `inserts emits event`() {
      val user100 = insertUser(100)

      store.insert(user100, true)

      eventPublisher.assertEventPublished(DefaultVoterAddedEvent(user100))
    }
  }
  ...
  inner class Delete {
    ...
    @Test
    fun `deletes emits event`() {
      val user100 = insertUser(100)
      insertDefaultVoter(user100)
      store.delete(user100, true)

      eventPublisher.assertEventPublished(DefaultVoterRemovedEvent(user100))
    }
  }
}
