...
class PublishedReportStoreTest : DatabaseTest(), RunsAsDatabaseUser {
  ...
  private lateinit var reportId1: ReportId
  private lateinit var reportId2: ReportId
  private lateinit var dealName: String
  private lateinit var commonIndicatorId1: CommonIndicatorId
  private lateinit var commonIndicatorId2: CommonIndicatorId
  private lateinit var projectIndicatorId1: ProjectIndicatorId
  private lateinit var projectIndicatorId2: ProjectIndicatorId
  ...
  fun setUp() {
    ...
    dealName = UUID.randomUUID().toString()
  }
    ...
    fun `only includes auto-calculated indicator targets for the report year and project`() {
      ...
      val expected =
          PublishedReportIndicatorModel(
              ...
              precision = AutoCalculatedIndicator.SurvivalRate.precision,
              progressNotes = null,
              projectsComments = null,
  ...
  @Nested
  inner class FetchPublishedReportsByIds {
    @Test
    fun `fetches one report by id`() {
      insertFullReportData()

      assertEquals(
          mapOf(
              reportId1 to expectedReport1(),
          ),
          store.fetchPublishedReportsByIds(listOf(reportId1)),
      )
    }

    @Test
    fun `fetches multiple reports by id`() {
      insertFullReportData()

      assertEquals(
          mapOf(
              reportId2 to expectedReport2(),
              reportId1 to expectedReport1(),
          ),
          store.fetchPublishedReportsByIds(listOf(reportId1, reportId2)),
      )
    }
  }

  private fun insertFullReportData() {
          ...
          autoCalculatedIndicators =
              listOf(
                  PublishedReportIndicatorModel(
                      ...
                      precision = AutoCalculatedIndicator.SeedsCollected.precision,
