...
CREATE TABLE accelerator.deliverable_statuses (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL UNIQUE
);
...
CREATE TABLE accelerator.project_deliverables (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id BIGINT NOT NULL REFERENCES projects ON DELETE CASCADE,
    deliverable_id BIGINT NOT NULL REFERENCES accelerator.deliverables,
    deliverable_status_id INTEGER NOT NULL REFERENCES accelerator.deliverable_statuses,
...
CREATE INDEX ON accelerator.project_deliverables (deliverable_id);
CREATE INDEX ON accelerator.project_deliverables (project_id);

CREATE TABLE accelerator.project_documents (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    project_id BIGINT NOT NULL REFERENCES projects ON DELETE CASCADE,
    deliverable_id BIGINT NOT NULL REFERENCES accelerator.deliverables,
    deliverable_type_id INTEGER NOT NULL REFERENCES accelerator.deliverable_types,
    ...
    FOREIGN KEY (project_id, deliverable_id)
        REFERENCES accelerator.project_deliverables (project_id, deliverable_id),
    FOREIGN KEY (deliverable_id, deliverable_type_id)
        REFERENCES accelerator.deliverables (id, deliverable_type_id)
);

CREATE INDEX ON accelerator.project_documents (deliverable_id);
CREATE INDEX ON accelerator.project_documents (project_id);
