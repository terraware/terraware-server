ALTER TABLE files ADD COLUMN file_date DATE;
ALTER TABLE files ADD COLUMN caption TEXT;
ALTER TABLE files ADD COLUMN width INTEGER;
ALTER TABLE files ADD COLUMN height INTEGER;

CREATE TABLE document_templates (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    name TEXT NOT NULL,

    UNIQUE (name)
);

CREATE TABLE variable_types (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL UNIQUE
);

CREATE TABLE variables (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    replaces_variable_id BIGINT REFERENCES variables ON DELETE SET NULL,
    is_list BOOLEAN NOT NULL DEFAULT FALSE,
    stable_id TEXT NOT NULL,
    name TEXT NOT NULL,
    description TEXT,

    CONSTRAINT cannot_have_multiple_replacements
        UNIQUE (replaces_variable_id),

    -- Compound unique key so child tables can constrain themselves to variables of the correct type.
    UNIQUE (id, variable_type_id)
);

CREATE TABLE variable_manifests (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    document_template_id BIGINT NOT NULL REFERENCES document_templates,
    created_by BIGINT NOT NULL DEFAULT -1 REFERENCES users ON DELETE SET DEFAULT,
    created_time TIMESTAMP WITH TIME ZONE NOT NULL
);

CREATE TABLE variable_manifest_entries (
    variable_manifest_id BIGINT NOT NULL REFERENCES variable_manifests,
    variable_id BIGINT NOT NULL REFERENCES variables,
    position INTEGER NOT NULL,

    PRIMARY KEY (variable_manifest_id, variable_id),
    UNIQUE (variable_manifest_id, position)
);

CREATE TABLE variable_numbers (
    variable_id BIGINT PRIMARY KEY REFERENCES variables ON DELETE CASCADE,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    min_value NUMERIC,
    max_value NUMERIC,
    decimal_places INTEGER NOT NULL DEFAULT 0,

    CHECK (variable_type_id = 1),

    FOREIGN KEY (variable_id, variable_type_id) REFERENCES variables (id, variable_type_id)
);

CREATE TABLE variable_text_types (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL UNIQUE
);

CREATE TABLE variable_texts (
    variable_id BIGINT PRIMARY KEY,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    variable_text_type_id INTEGER NOT NULL REFERENCES variable_text_types,

    CHECK (variable_type_id = 2),

    FOREIGN KEY (variable_id, variable_type_id) REFERENCES variables (id, variable_type_id) ON DELETE CASCADE
);

CREATE TABLE variable_images (
    variable_id BIGINT PRIMARY KEY,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    title_required BOOLEAN NOT NULL,

    CHECK (variable_type_id = 4),

    FOREIGN KEY (variable_id, variable_type_id) REFERENCES variables (id, variable_type_id) ON DELETE CASCADE
);

CREATE TABLE variable_sections (
    variable_id BIGINT PRIMARY KEY,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    parent_variable_id BIGINT,
    parent_variable_type_id INTEGER REFERENCES variable_types,
    position INTEGER NOT NULL,
    render_heading BOOLEAN NOT NULL DEFAULT TRUE,

    CHECK (variable_type_id = 8),
    CHECK (parent_variable_type_id IS NULL OR parent_variable_type_id = 8),

    CONSTRAINT no_duplicate_positions_under_parent
        UNIQUE (parent_variable_id, position),

    FOREIGN KEY (variable_id, variable_type_id) REFERENCES variables (id, variable_type_id) ON DELETE CASCADE,
    FOREIGN KEY (parent_variable_id, parent_variable_type_id) REFERENCES variables (id, variable_type_id) ON DELETE CASCADE
);

CREATE TABLE variable_selects (
    variable_id BIGINT PRIMARY KEY,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    is_multiple BOOLEAN NOT NULL,

    CHECK (variable_type_id = 5),

    UNIQUE (variable_id, variable_type_id),

    FOREIGN KEY (variable_id, variable_type_id) REFERENCES variables (id, variable_type_id) ON DELETE CASCADE
);

CREATE TABLE variable_select_options (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    variable_id BIGINT NOT NULL REFERENCES variables ON DELETE CASCADE,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    position INTEGER NOT NULL,
    name TEXT NOT NULL,
    description TEXT,

    UNIQUE (id, variable_id),

    FOREIGN KEY (variable_id, variable_type_id) REFERENCES variable_selects (variable_id, variable_type_id) ON DELETE CASCADE
);

CREATE TABLE variable_table_columns (
    variable_id BIGINT PRIMARY KEY REFERENCES variables,
    table_variable_id BIGINT NOT NULL REFERENCES variables,
    table_variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    position INTEGER NOT NULL,

    CHECK (table_variable_type_id = 6),

    CONSTRAINT no_duplicate_column_positions
        UNIQUE (table_variable_id, position),

    FOREIGN KEY (table_variable_id, table_variable_type_id) REFERENCES variable_selects (variable_id, variable_type_id) ON DELETE CASCADE
);

CREATE TABLE variable_impacts (
    impacting_variable_id BIGINT NOT NULL REFERENCES variables ON DELETE CASCADE,
    impacted_variable_id BIGINT NOT NULL REFERENCES variables ON DELETE CASCADE,

    PRIMARY KEY (impacting_variable_id, impacted_variable_id),
    UNIQUE (impacted_variable_id, impacting_variable_id)
);
CREATE TABLE pdd_statuses (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL UNIQUE
);

CREATE TABLE pdds (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    document_template_id BIGINT NOT NULL REFERENCES document_templates,
    variable_manifest_id BIGINT NOT NULL REFERENCES variable_manifests,
    name TEXT NOT NULL,
    organization_name TEXT NOT NULL,
    status_id INTEGER NOT NULL REFERENCES pdd_statuses,
    owned_by BIGINT NOT NULL DEFAULT -1 REFERENCES users ON DELETE SET DEFAULT,
    created_by BIGINT NOT NULL DEFAULT -1 REFERENCES users ON DELETE SET DEFAULT,
    created_time TIMESTAMP WITH TIME ZONE NOT NULL,
    modified_by BIGINT NOT NULL DEFAULT -1 REFERENCES users ON DELETE SET DEFAULT,
    modified_time TIMESTAMP WITH TIME ZONE NOT NULL,

    UNIQUE (name)
);

CREATE TABLE pdd_saved_versions (
    pdd_id BIGINT NOT NULL REFERENCES pdds ON DELETE CASCADE,
    variable_manifest_id BIGINT NOT NULL REFERENCES variable_manifests,
    max_variable_value_id BIGINT NOT NULL,
    created_by BIGINT NOT NULL DEFAULT -1 REFERENCES users ON DELETE SET DEFAULT,
    created_time TIMESTAMP WITH TIME ZONE NOT NULL,
    name TEXT,
    is_submitted BOOLEAN NOT NULL DEFAULT FALSE,
    is_accepted BOOLEAN NOT NULL DEFAULT FALSE
);

CREATE TABLE variable_values (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    pdd_id BIGINT NOT NULL REFERENCES pdds ON DELETE CASCADE,
    variable_id BIGINT NOT NULL REFERENCES variables,
    variable_type_id BIGINT NOT NULL REFERENCES variable_types,
    index INTEGER NOT NULL DEFAULT 0,
    created_by BIGINT NOT NULL DEFAULT -1 REFERENCES users ON DELETE SET DEFAULT,
    created_time TIMESTAMP WITH TIME ZONE NOT NULL,
    verified_by BIGINT DEFAULT -1 REFERENCES users ON DELETE SET DEFAULT,
    verified_time TIMESTAMP WITH TIME ZONE,
    number_value NUMERIC,
    text_value TEXT,
    date_value DATE,

    CHECK (number_value IS NULL OR variable_type_id = 1),
    CHECK (text_value IS NULL OR variable_type_id = 2),

    -- Unique keys to allow child tables to enforce that values are for the right variables.
    UNIQUE (variable_id, id),
    UNIQUE (variable_id, variable_type_id, id),
    UNIQUE (pdd_id, id),

    FOREIGN KEY (variable_id, variable_type_id) REFERENCES variables (id, variable_type_id)
);

CREATE TABLE variable_image_values (
    variable_value_id BIGINT PRIMARY KEY REFERENCES variable_values ON DELETE CASCADE,
    variable_id BIGINT NOT NULL REFERENCES variables,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    storage_url TEXT NOT NULL,
    caption TEXT,
    description TEXT,

    CHECK (variable_type_id = 4),

    FOREIGN KEY (variable_value_id, variable_id, variable_type_id) REFERENCES variable_values (id, variable_id, variable_type_id)
);

CREATE TABLE variable_link_values (
    variable_value_id BIGINT PRIMARY KEY REFERENCES variable_values ON DELETE CASCADE,
    variable_id BIGINT NOT NULL REFERENCES variables,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    url TEXT NOT NULL,
    title TEXT NOT NULL,

    CHECK (variable_type_id = 7),

    FOREIGN KEY (variable_value_id, variable_id, variable_type_id) REFERENCES variable_values (id, variable_id, variable_type_id)
);

CREATE TABLE variable_section_values (
    variable_value_id BIGINT PRIMARY KEY REFERENCES variable_values ON DELETE CASCADE,
    variable_id BIGINT NOT NULL REFERENCES variables,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    text_value TEXT,
    referenced_variable_id BIGINT REFERENCES variables,
    referenced_variable_type_id INTEGER NOT NULL REFERENCES variable_types,

    CONSTRAINT cannot_reference_other_sections
        CHECK (referenced_variable_type_id IS NULL OR referenced_variable_type_id <> 8),
    CONSTRAINT cannot_have_both_text_and_reference
        CHECK (text_value IS NULL OR referenced_variable_id IS NULL),
    CONSTRAINT must_have_text_or_reference
        CHECK (text_value IS NOT NULL OR referenced_variable_id IS NOT NULL),

    FOREIGN KEY (referenced_variable_id, referenced_variable_type_id) REFERENCES variables (id, variable_type_id),
    FOREIGN KEY (variable_value_id, variable_id, variable_type_id) REFERENCES variable_values (id, variable_id, variable_type_id)
);

CREATE INDEX ON variable_section_values (referenced_variable_id);

CREATE TABLE variable_select_option_values (
    variable_value_id BIGINT REFERENCES variable_values ON DELETE CASCADE,
    variable_id BIGINT NOT NULL REFERENCES variable_selects,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    option_id BIGINT NOT NULL REFERENCES variable_select_options,

    PRIMARY KEY (variable_value_id, option_id),

    CHECK (variable_type_id = 5),

    FOREIGN KEY (option_id, variable_id) REFERENCES variable_select_options (id, variable_id),
    FOREIGN KEY (variable_value_id, variable_id, variable_type_id) REFERENCES variable_values (id, variable_id, variable_type_id)
);

CREATE TABLE variable_value_table_rows (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    variable_value_id BIGINT NOT NULL REFERENCES variable_values ON DELETE CASCADE,
    table_row_value_id BIGINT NOT NULL REFERENCES variable_values ON DELETE CASCADE,

    UNIQUE (variable_value_id, table_row_value_id)
);

CREATE INDEX ON variable_value_table_rows (table_row_value_id);

CREATE VIEW variable_current_values AS
    SELECT DISTINCT ON (variable_id, index)
        *
    FROM variable_values
    ORDER BY variable_id, index, id DESC;

CREATE TABLE variable_value_citations (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    variable_value_id BIGINT NOT NULL REFERENCES variable_values ON DELETE CASCADE,
    name TEXT NOT NULL,
    url TEXT
);

CREATE INDEX ON variable_value_citations (variable_value_id);
CREATE FUNCTION reject_delete() RETURNS TRIGGER AS $$
BEGIN
    RAISE 'This table does not allow deletes.';
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION reject_update() RETURNS TRIGGER AS $$
BEGIN
    RAISE 'This table does not allow updates.';
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION reject_delete_value() RETURNS TRIGGER AS $$
BEGIN
    IF EXISTS (SELECT 1 FROM pdds WHERE id = OLD.pdd_id) THEN
        RAISE 'This table does not allow deletes.';
    ELSE
        -- The entire PDD is being deleted.
        RETURN OLD;
    END IF;
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION reject_update_value() RETURNS TRIGGER AS $$
BEGIN
    IF (OLD.created_by != -1 AND NEW.created_by = -1
        AND NOT EXISTS (SELECT 1 FROM users WHERE id = OLD.created_by))
        OR (OLD.verified_by != -1 AND NEW.verified_by = -1
            AND NOT EXISTS (SELECT 1 FROM users WHERE id = OLD.verified_by))
    THEN
        RAISE 'This table does not allow updates.';
    ELSE
        -- This is an update triggered by a user being deleted.
        RETURN NEW;
    END IF;
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION reject_delete_value_child() RETURNS TRIGGER AS $$
BEGIN
    IF EXISTS (SELECT 1 FROM variable_values WHERE id = OLD.variable_value_id) THEN
        RAISE 'This table does not allow deletes.';
    ELSE
        RETURN OLD;
    END IF;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER variable_values_no_update
    BEFORE UPDATE ON variable_values
    FOR EACH ROW EXECUTE FUNCTION reject_update_value();

CREATE TRIGGER variable_values_no_delete
    BEFORE DELETE ON variable_values
    FOR EACH ROW EXECUTE FUNCTION reject_delete_value();

CREATE TRIGGER variable_image_values_no_update
    BEFORE UPDATE ON variable_image_values
    FOR EACH ROW EXECUTE FUNCTION reject_update();

CREATE TRIGGER variable_image_values_no_delete
    BEFORE DELETE ON variable_image_values
    FOR EACH ROW EXECUTE FUNCTION reject_delete_value_child();

CREATE TRIGGER variable_section_values_no_update
    BEFORE UPDATE ON variable_section_values
    FOR EACH ROW EXECUTE FUNCTION reject_update();

CREATE TRIGGER variable_section_values_no_delete
    BEFORE DELETE ON variable_section_values
    FOR EACH ROW EXECUTE FUNCTION reject_delete_value_child();

CREATE TRIGGER variable_select_option_values_no_update
    BEFORE UPDATE ON variable_select_option_values
    FOR EACH ROW EXECUTE FUNCTION reject_update();

CREATE TRIGGER variable_select_option_values_no_delete
    BEFORE DELETE ON variable_select_option_values
    FOR EACH ROW EXECUTE FUNCTION reject_delete_value_child();

CREATE TRIGGER variables_no_update
    BEFORE UPDATE ON variables
    FOR EACH ROW EXECUTE FUNCTION reject_update();

CREATE TRIGGER variables_no_delete
    BEFORE DELETE ON variables
    FOR EACH ROW EXECUTE FUNCTION reject_delete();

DROP TABLE variable_impacts;

CREATE TABLE variable_impacts (
    impacting_variable_id BIGINT NOT NULL REFERENCES variables ON DELETE CASCADE,
    impacted_variable_id BIGINT NOT NULL REFERENCES variables ON DELETE CASCADE,
    variable_manifest_id BIGINT NOT NULL REFERENCES variable_manifests ON DELETE CASCADE,

    PRIMARY KEY (impacting_variable_id, variable_manifest_id, impacted_variable_id)
);

CREATE UNIQUE INDEX ON variable_impacts (impacted_variable_id, variable_manifest_id, impacting_variable_id);
ALTER TABLE variable_select_options ADD COLUMN rendered_text TEXT;

CREATE TABLE variable_table_styles (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL UNIQUE
);

CREATE TABLE variable_tables (
    variable_id BIGINT PRIMARY KEY,
    variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    table_style_id INTEGER NOT NULL REFERENCES variable_table_styles,

    CHECK (variable_type_id = 6),

    FOREIGN KEY (variable_id, variable_type_id) REFERENCES variables (id, variable_type_id) ON DELETE CASCADE
);

DROP VIEW variable_current_values;

ALTER TABLE variable_values DROP COLUMN verified_by;
ALTER TABLE variable_values RENAME COLUMN verified_time TO completed_time;

CREATE VIEW variable_current_values AS
    SELECT DISTINCT ON (variable_id, index)
        *
    FROM variable_values
    ORDER BY variable_id, index, id DESC;

-- No longer need the "title required" flag which was the only attribute in this table.
DROP TABLE variable_images;
CREATE TABLE variable_usage_types (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL UNIQUE
);

ALTER TABLE variable_section_values ADD COLUMN usage_type_id INTEGER REFERENCES variable_usage_types;
ALTER TABLE variable_section_values RENAME referenced_variable_id TO used_variable_id;
ALTER TABLE variable_section_values RENAME referenced_variable_type_id TO used_variable_type_id;

ALTER TABLE variable_section_values ADD CONSTRAINT usages_must_have_types
    CHECK ((used_variable_id IS NULL) = (usage_type_id IS NULL));

CREATE TABLE variable_injection_display_styles (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL UNIQUE
);

ALTER TABLE variable_section_values ADD COLUMN display_style_id INTEGER REFERENCES variable_injection_display_styles;

-- Also in R__TypeCodes.sql, but duplicated here to clarify what the constraint means
INSERT INTO variable_usage_types (id, name)
VALUES (1, 'Injection'),
       (2, 'Reference');

ALTER TABLE variable_section_values ADD CONSTRAINT injection_has_display_style
    CHECK ((usage_type_id = 1) = (display_style_id IS NOT NULL));

-- 4 = image, 6 = table, 8 = section
ALTER TABLE variable_section_values ADD CONSTRAINT reference_only_for_figures_and_sections
    CHECK (usage_type_id <> 2 OR used_variable_type_id IN (4, 6, 8));

ALTER TABLE variable_section_values ADD CONSTRAINT cannot_inject_sections
    CHECK (NOT (usage_type_id = 1 AND used_variable_type_id = 8));
ALTER TABLE variable_table_columns DROP CONSTRAINT variable_table_columns_table_variable_id_table_variable_ty_fkey;

ALTER TABLE variable_tables ADD UNIQUE (variable_id, variable_type_id);

ALTER TABLE variable_table_columns ADD
    FOREIGN KEY (table_variable_id, table_variable_type_id)
    REFERENCES variable_tables (variable_id, variable_type_id)
    ON DELETE CASCADE;
-- Positions of child variables are determined by manifest positions.
ALTER TABLE variable_sections DROP COLUMN position;
ALTER TABLE variable_table_columns DROP COLUMN position;
DROP VIEW variable_current_values;

ALTER TABLE variable_values DROP COLUMN completed_time;

CREATE VIEW variable_current_values AS
    SELECT DISTINCT ON (variable_id, index)
        id,
        pdd_id,
        variable_id,
        variable_type_id,
        index,
        created_by,
        created_time,
        number_value,
        text_value,
        date_value
    FROM variable_values
    ORDER BY variable_id, index, id DESC;
-- This can be null for text values.
ALTER TABLE variable_section_values ALTER COLUMN used_variable_type_id DROP NOT NULL;

-- Not using this view anywhere and it gets in the way of ALTER TABLE statements.
DROP VIEW variable_current_values;

-- Variable type ID needs to be an integer, not a bigint, so codegen will turn it into an enum.
ALTER TABLE variable_values ALTER COLUMN variable_type_id TYPE INTEGER;

ALTER TABLE variable_values ADD COLUMN is_deleted BOOLEAN NOT NULL DEFAULT FALSE;

ALTER TABLE variable_values RENAME COLUMN index TO list_position;
-- Use the generic "files" table for image information, but caption can change over time so
-- needs to be pulled from the variable value.
ALTER TABLE variable_image_values ADD COLUMN file_id BIGINT NOT NULL REFERENCES files;
ALTER TABLE variable_image_values DROP COLUMN storage_url;
ALTER TABLE variable_image_values DROP COLUMN description;

ALTER TABLE files DROP COLUMN caption;
DROP TABLE variable_value_citations;

ALTER TABLE variable_values ADD COLUMN citation TEXT;
ALTER TABLE variable_table_columns ADD COLUMN is_header BOOLEAN;
UPDATE variable_table_columns SET is_header = FALSE;
ALTER TABLE variable_table_columns ALTER COLUMN is_header SET NOT NULL;
ALTER TABLE pdd_saved_versions ADD COLUMN id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY;
CREATE TABLE variable_section_recommendations (
    section_variable_id BIGINT NOT NULL REFERENCES variables ON DELETE CASCADE,
    section_variable_type_id INTEGER NOT NULL REFERENCES variable_types,
    recommended_variable_id BIGINT NOT NULL REFERENCES variables ON DELETE CASCADE,
    variable_manifest_id BIGINT NOT NULL REFERENCES variable_manifests ON DELETE CASCADE,

    PRIMARY KEY (section_variable_id, recommended_variable_id),
    UNIQUE (recommended_variable_id, variable_manifest_id, section_variable_id),

    CHECK (section_variable_type_id = 8)
);

INSERT INTO variable_section_recommendations
SELECT impacting_variable_id, impacted_variable_id, variable_manifest_id
FROM variable_impacts;

DROP TABLE variable_impacts;

ALTER TABLE variable_section_recommendations DROP CONSTRAINT variable_section_recommendations_pkey;
ALTER TABLE variable_section_recommendations ADD PRIMARY KEY (section_variable_id, variable_manifest_id, recommended_variable_id);

ALTER TABLE variable_link_values ALTER COLUMN title DROP NOT NULL;

ALTER TABLE pdds RENAME TO documents;
ALTER TABLE pdd_statuses RENAME TO document_statuses;
ALTER TABLE pdd_saved_versions RENAME TO document_saved_versions;

ALTER TABLE document_saved_versions RENAME pdd_id TO document_id;
ALTER TABLE variable_values RENAME pdd_id TO document_id;
