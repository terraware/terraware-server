...
COMMENT ON TABLE file_batch_statuses IS '(Enum) Statuses of file batches.';
...
COMMENT ON COLUMN files.file_batch_id IS 'If this file was uploaded as part of a batch, the ID of the batch.';
...
COMMENT ON TABLE species_native_categories IS '(Enum) Categories related to native-ness of a species. Used for accelerator species management.';

COMMENT ON TABLE species_nativities IS '(Enum) Native/invasive/introduced values for a species in a given region.';
...
COMMENT ON TABLE splat_annotation_media IS 'Media files (photos or videos) attached to splat annotations. Modeled for multiple media per annotation; currently at most one.';
...
COMMENT ON TABLE tracking.planting_seasons IS 'Named planting seasons for planting sites, with start and end dates.';
...
COMMENT ON TABLE tracking.scheduled_planting_dates IS 'Planned planting dates within a planting season.';
