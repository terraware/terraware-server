...
COMMENT ON TABLE accelerator.deliverable_statuses IS '(Enum) Statuses of deliverables in the context of a particular project.';
...
COMMENT ON TABLE accelerator.project_deliverables IS 'Information about the current states of the deliverables for particular projects.';

COMMENT ON TABLE accelerator.project_documents IS 'Information about documents uploaded by users to satisfy deliverables. A deliverable can have multiple documents.';
COMMENT ON COLUMN accelerator.project_documents.name IS 'System-generated filename. This includes several elements such as the date and description.';
COMMENT ON COLUMN accelerator.project_documents.location IS 'Location of file in the document store identified by `document_store_id`. This is used by the system to generate download links and includes whatever information is needed to generate a link for a given document store; if the document store supports permalinks then this may be a simple URL.';
