ALTER TABLE nursery.batches
    ADD COLUMN hardening_off_quantity                 INTEGER DEFAULT 0,
    ADD COLUMN latest_observed_hardening_off_quantity INTEGER DEFAULT 0
;
...
ALTER TABLE nursery.batch_withdrawals
    ADD COLUMN hardening_off_quantity_withdrawn INTEGER DEFAULT 0
;

-- add hardening_off_quantity, previous version was V140
...
-- add hardening_off_quantity, previous version was V222
DROP VIEW nursery.facility_inventory_totals;
...
SELECT batches.organization_id,
       ...
       sum(batches.ready_quantity + batches.not_ready_quantity +
...
-- add hardening_off_quantity_withdrawn to totals calculation, previous version was V261
...
FROM nursery.withdrawals withdrawals
         ...
         LEFT JOIN LATERAL ( SELECT COALESCE(sum(bw.germinating_quantity_withdrawn +
                                                 ...
                                                 bw.not_ready_quantity_withdrawn +
...
-- add hardening_off_quantity, previous version was V234
