...
class ObservationResultsStore(private val dslContext: DSLContext) {
                    ...
                    DSL.coalesce(
                        ...
                        DSL.`when`(
                            PLOT_T0_DENSITIES.PLOT_DENSITY.isNotNull.or(
                                PLANTING_ZONE_T0_TEMP_DENSITIES.ZONE_DENSITY.isNotNull
                            ),
  ...
  //  private val monitoringPlotSpeciesMultiset: Field<List<ObservationSpeciesResultsModel>> =
  //      // todo this isn't working, it's not returning the correct rows from
  //      //   observed_plot_species_totals for some reason
  //      with(OBSERVED_PLOT_SPECIES_TOTALS) {
  //        val allPlotSpeciesCombinations =
  //            DSL.select(
  //                    MONITORING_PLOTS.ID.`as`("monitoring_plot_id"),
  //                    DSL.coalesce(
  ...
  //                .leftJoin(OBSERVED_PLOT_SPECIES_TOTALS)
  ...
  //                .on(PLOT_T0_DENSITIES.MONITORING_PLOT_ID.eq(MONITORING_PLOTS.ID))
  ...
  //                )
  //                .where(
  //                    OBSERVED_PLOT_SPECIES_TOTALS.SPECIES_ID.isNotNull
  //                        .or(PLOT_T0_DENSITIES.SPECIES_ID.isNotNull)
  //                        .or(PLANTING_ZONE_T0_TEMP_DENSITIES.SPECIES_ID.isNotNull)
  //                )
  //                .asTable("plot_species_combinations")
  //
  //        val nullSpeciesObservations =
  //            DSL.select(
  //                    OBSERVED_PLOT_SPECIES_TOTALS.MONITORING_PLOT_ID.`as`("monitoring_plot_id"),
  //                    DSL.inline(null as Int?).`as`("species_id"),
  //                )
  //                .from(OBSERVED_PLOT_SPECIES_TOTALS)
  //                .where(OBSERVED_PLOT_SPECIES_TOTALS.SPECIES_ID.isNull)
  //                .asTable("null_species_obs")
  //
  //        val allCombinations =
  //            DSL.select(allPlotSpeciesCombinations.asterisk())
  //                .from(allPlotSpeciesCombinations)
  //
  // .union(DSL.select(nullSpeciesObservations.asterisk()).from(nullSpeciesObservations))
  //                .asTable()
  //        val monitoringPlotIdField =
  //            allCombinations.field("monitoring_plot_id", MonitoringPlotId::class.java)
  //        val speciesIdField = allCombinations.field("species_id", SpeciesId::class.java)
  //
  //        speciesMultiset(
  //            DSL.select(
  //                    DSL.coalesce(CERTAINTY_ID, RecordedSpeciesCertainty.Known),
  ...
  //                        )
  //                        .`when`(
  //                            monitoringPlots.plantingSites.SURVIVAL_RATE_INCLUDES_TEMP_PLOTS.eq(
  //                                true
  //                            ),
  //                            DSL.select(PLANTING_ZONE_T0_TEMP_DENSITIES.ZONE_DENSITY)
  ...
  //                .leftJoin(OBSERVED_PLOT_SPECIES_TOTALS)
  //
  // .on(MONITORING_PLOT_ID.eq(monitoringPlotIdField).and(SPECIES_ID.eq(speciesIdField)))
  //                // full outer join because we want survival rate to be 0 if a species wasn't
  //                // observed but has t0 density data set
  //                .leftJoin(PLOT_T0_DENSITIES)
  //                .on(
  //                    PLOT_T0_DENSITIES.MONITORING_PLOT_ID.eq(monitoringPlotIdField)
  //                        .and(PLOT_T0_DENSITIES.SPECIES_ID.eq(speciesIdField))
  //                        .and(OBSERVATION_PLOTS.IS_PERMANENT.eq(true))
  //                )
  //                .leftJoin(MONITORING_PLOT_HISTORIES)
  //                .on(MONITORING_PLOT_HISTORIES.MONITORING_PLOT_ID.eq(monitoringPlotIdField))
  //                .leftJoin(PLANTING_ZONE_T0_TEMP_DENSITIES)
  //                .on(
  //                    PLANTING_ZONE_T0_TEMP_DENSITIES.PLANTING_ZONE_ID.eq(
  //                            MONITORING_PLOT_HISTORIES.monitoringPlots.plantingSubzones
  //                                .PLANTING_ZONE_ID
  //                        )
  //                        .and(PLANTING_ZONE_T0_TEMP_DENSITIES.SPECIES_ID.eq(speciesIdField))
  //                        .and(OBSERVATION_PLOTS.IS_PERMANENT.eq(false))
  //                )
  //                .where(
  //                    monitoringPlotIdField!!
  //                        .eq(MONITORING_PLOTS.ID)
  ...
  //                        )
  //                        .and(
  //                            PLOT_T0_DENSITIES.SPECIES_ID.eq(SPECIES_ID)
  //                            //                            .or(SPECIES_ID.isNull)
  //                        )
  //                        .and(OBSERVATION_PLOTS.IS_PERMANENT.eq(true))
  //                )
  //                .leftJoin(PLANTING_ZONE_T0_TEMP_DENSITIES)
  ...
  //                        )
  //                        .and(
  //                            PLANTING_ZONE_T0_TEMP_DENSITIES.SPECIES_ID.eq(SPECIES_ID)
  //                            //                                .or(SPECIES_ID.isNull)
  //                        )
  //                        .and(OBSERVATION_PLOTS.IS_PERMANENT.eq(false))
  //                )
  ...
  //                //
  //                //
  // .and(MONITORING_PLOT_HISTORIES.ID.eq(OBSERVATION_PLOTS.MONITORING_PLOT_HISTORY_ID))
  //                .and(OBSERVATION_ID.eq(OBSERVATIONS.ID).or(OBSERVATION_ID.isNull))
  //                .orderBy(SPECIES_ID, SPECIES_NAME)
  //        )
  //      }
  ...
  //      with(OBSERVED_PLOT_SPECIES_TOTALS) {
  //        val plotSpecies =
  //            DSL.selectDistinct(MONITORING_PLOT_ID, SPECIES_ID)
  //                .from(this)
  //                .union(
  //                    DSL.selectDistinct(
  //                            MONITORING_PLOT_ID,
  ...
  //                )
  //                .asTable("all_species")
  //        val plotIdField = plotSpecies.field(MONITORING_PLOT_ID)
  //        val speciesIdField = plotSpecies.field(SPECIES_ID)
  //
  //        speciesMultiset(
  //            DSL.select(
  //                    DSL.coalesce(CERTAINTY_ID, RecordedSpeciesCertainty.Known),
  ...
  //                )
  //                .from(MONITORING_PLOT_HISTORIES)
  //                .leftJoin(plotSpecies)
  //                .on(MONITORING_PLOT_HISTORIES.MONITORING_PLOT_ID.eq(plotIdField))
  //                //                .from(plotSpecies)
  //                //                .innerJoin(MONITORING_PLOT_HISTORIES)
  //                //
  // .on(MONITORING_PLOT_HISTORIES.MONITORING_PLOT_ID.eq(plotIdField))
  //                .leftJoin(OBSERVED_PLOT_SPECIES_TOTALS)
  //                .on(
  //                    OBSERVED_PLOT_SPECIES_TOTALS.MONITORING_PLOT_ID.eq(plotIdField)
  //                        .and(OBSERVED_PLOT_SPECIES_TOTALS.SPECIES_ID.eq(speciesIdField))
  //                )
  //                .leftJoin(PLOT_T0_DENSITIES)
  //                .on(
  ...
  //                .where(
  //
  // MONITORING_PLOT_HISTORIES.ID.eq(OBSERVATION_PLOTS.MONITORING_PLOT_HISTORY_ID)
  //                )
  //                // .and(speciesIdField?.isNotNull)
  //                .and(OBSERVATION_ID.eq(OBSERVATIONS.ID).or(OBSERVATION_ID.isNull))
  //                .orderBy(speciesIdField, SPECIES_NAME)
  //        )
  //      }
  ...
  private fun plantingSubzoneSpeciesMultiset(): Field<List<ObservationSpeciesResultsModel>> {
    ...
    val permDensityCol = permanentSubzoneT0.field("plot_density", BigDecimal::class.java)!!
    ...
    val tempDensityCol = tempSubzoneT0.field("plot_density", BigDecimal::class.java)!!
          ...
          DSL.select(
              ...
              .where(
                  PLANTING_SUBZONE_ID.eq(PLANTING_SUBZONE_HISTORIES.PLANTING_SUBZONE_ID)
                      ...
                      .or(tempSubzoneCol.eq(PLANTING_SUBZONE_HISTORIES.PLANTING_SUBZONE_ID))
              )
              .and(OBSERVATION_ID.eq(OBSERVATIONS.ID).or(OBSERVATION_ID.isNull))
              .orderBy(SPECIES_ID, SPECIES_NAME)
      )
  ...
  private fun plantingSubzonesMultiset(
    ...
    return DSL.multiset(
            DSL.select(
                    ...
                    PLANTING_SUBZONE_HISTORIES.plantingZoneHistories.plantingSiteHistories
                        .plantingSites
  ...
  private fun plantingZoneSpeciesMultiset(): Field<List<ObservationSpeciesResultsModel>> {
    ...
    val permDensityCol = permZoneT0.field("plot_density", BigDecimal::class.java)!!
    ...
    val tempDensityCol = tempZoneT0.field("plot_density", BigDecimal::class.java)!!
          ...
          DSL.select(
              ...
              .where(
                  PLANTING_ZONE_ID.eq(PLANTING_ZONE_HISTORIES.PLANTING_ZONE_ID)
                      ...
                      .or(tempZoneCol.eq(PLANTING_ZONE_HISTORIES.PLANTING_ZONE_ID))
              )
              .and(OBSERVATION_ID.eq(OBSERVATIONS.ID).or(OBSERVATION_ID.isNull))
              .orderBy(SPECIES_ID, SPECIES_NAME)
      )
  ...
  private fun plantingZoneMultiset(
    ...
    return DSL.multiset(
            DSL.select(
                    ...
                    PLANTING_ZONE_HISTORIES.plantingSiteHistories.plantingSites
                        .SURVIVAL_RATE_INCLUDES_TEMP_PLOTS,
                )
