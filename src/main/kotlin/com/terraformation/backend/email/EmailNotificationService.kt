...
class EmailNotificationService(
  ...
  private val accessionDatePendingEmails: MutableList<EmailRequest> = mutableListOf<EmailRequest>()
  private val accessionStatePendingEmails: MutableList<EmailRequest> = mutableListOf<EmailRequest>()
  ...
  private fun sendEmail(emailRequest: EmailRequest) {
    with(emailRequest) {
      try {
        emailService.sendUserNotification(user, emailTemplate, emailTemplateModel)
      } catch (e: Exception) {
        log.error("Error sending email $emailTemplate to user ${user.email}", e)
      }
    }
  }
