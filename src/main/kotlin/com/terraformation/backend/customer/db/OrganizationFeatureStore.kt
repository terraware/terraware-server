...
@Named
class OrganizationFeatureStore(
  ...
  private val applicationsExistsField = DSL.field(DSL.exists(
      DSL.selectOne()
          .from(APPLICATIONS)
          .join(PROJECTS)
  ...
  private val deliverablesExistsField = DSL.field(DSL.exists(
      DSL.selectOne()
          .from(DELIVERABLES)
  ...
  private val modulesExistsField = DSL.field(DSL.exists(
      DSL.selectOne()
          .from(COHORT_MODULES)
          .join(PARTICIPANTS)
          .on(PARTICIPANTS.COHORT_ID.eq(COHORT_MODULES.COHORT_ID))
  ...
  private val reportsExistsField = DSL.field(DSL.exists(
      DSL.selectOne()
          .from(REPORTS)
          .join(PROJECTS)
  ...
  private val seedFundReportsExistsField = DSL.field(DSL.exists(
      DSL.selectOne()
