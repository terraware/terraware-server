...
class DocumentTemplatesController(
  ...
  fun listDocumentTemplates(): ListDocumentTemplatesResponsePayload {
    val rows = documentTemplatesDao.findAll()

    return ListDocumentTemplatesResponsePayload(rows.map { DocumentTemplatePayload(it) })
  }
}
...
data class DocumentTemplatePayload(
  ...
  constructor(row: DocumentTemplatesRow) : this(id = row.id!!, name = row.name!!)
}
