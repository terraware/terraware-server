  ...
  override val sublists: List<SublistField> by lazy {
    with(tables) {
      listOf(
          ...
          accessions.asMultiValueSublist("accessions", PROJECTS.ID, ACCESSIONS.PROJECT_ID),
          applications.asSingleValueSublist("application", PROJECTS.ID, APPLICATIONS.PROJECT_ID),
          batches.asMultiValueSublist("batches", PROJECTS.ID, BATCHES.PROJECT_ID),
          botanicalCountries.asSingleValueSublist(
              "botanicalCountry",
              PROJECTS.BOTANICAL_COUNTRY_CODE,
              BOTANICAL_COUNTRIES.LEVEL3_CODE,
          ),
          countries.asSingleValueSublist("country", PROJECTS.COUNTRY_CODE, COUNTRIES.CODE),
          documents.asMultiValueSublist("documents", PROJECTS.ID, DOCUMENTS.PROJECT_ID),
